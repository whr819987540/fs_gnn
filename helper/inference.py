...
# 用全图从第一层开始前向传播
def sample_full(adj: torch.Tensor, layer_num: int, importance: bool):
    """
    adj: 全图(没有进行划分)上的邻接矩阵。对角线上是1
    layer_num:GNN模型层数,不包括FS层
    importance: 为True代表是重要性采样(layer_importance_sampling),为False代表是随机采样(layer_wise_sampling或node_wise_sampling)

    返回的adjs用于前向传播中
    previous_indices只有在node采样配套graphsage_first模型时使用,两种层采样配套GCN_first模型时不用管这个返回值
    """
    if importance:   # 因为是重要性采样,所以在测试时adj也要按照重要性的方式归一化,所以这里用normalize
        adj = normalize(adj)
    else:
