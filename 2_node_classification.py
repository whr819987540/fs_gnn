...
def run(proc_id, devices, args, log_id, graph, train_nids, valid_nids, test_nids):
    ...
    for epoch in range(args.n_epochs):
        ...
        if proc_id == 0:
            model_copy = create_model(args, deepcopy(layer_size), device)
            model_copy.load_state_dict(model.module.state_dict())
            
            predictions = []
            labels = []
            test_acc, train_loss = get_acc_loss(test_dataloader, model)
            train_acc, test_loss = get_acc_loss(train_dataloader, model)
            val_acc, val_loss = get_acc_loss(valid_dataloader, model)
